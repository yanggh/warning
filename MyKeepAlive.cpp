...
int  RecvUdp()
{
    ...
    while(1)
    {
        bzero(message, 1024);
        size_t  len = 1024;
        iLen = recvfrom(sockfd,  message, len, 0, (struct sockaddr*)&cin, (socklen_t*)&cin_len);
        ...
        if(iLen > 0)
        {
            ...
            if(type == WARING) 
            {
                sendto(sockfd, (uint8_t*)&fnum, 2, 0, (struct sockaddr *)&cin, cin_len);
                decomp((uint8_t*)message, iLen, jsonstr, &outlen);
                add_json_str(jsonstr);
            }
            else if(type == KEEPALIVE)
            {
                CheckClient(inet_ntoa(cin.sin_addr), ntohs(cin.sin_port));
