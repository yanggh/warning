...
void  Song()
{
    ...
    snprintf(constr, 256, "tcp://%s:7777", ip.c_str());

    syslog(LOG_INFO, "web ip is %s", ip.c_str());
    ...
    while(1)
    {
        ...
        else if(0 == strncmp((char*)request.data(), "update",strlen("update")))
        {
        ...
        else
        {
            ...
            if(-1 == sscanf((const char*) request.data(), "%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d%*[^0-9]%d", &fatal_s, &fatal, &serious_s, &serious, &common_s, &common))
            {
